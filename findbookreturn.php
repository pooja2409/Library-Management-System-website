...
oci_execute($stid);
    ...
    //echo $row[0] . " " . $row[1] .  " " . $row[2] ." " . $row[3] ." " . $row[4] ." " . $row[5] . "<br>\n";
		...
		$available=$row[6];
			...
			header("Location:returnbookscheck.php?booksisuedyet=$no&user=$name&titleout=$title&isbnout=$isbn&authorout=$author&publisherout=$publisher&priceout=$price&quantityout=$quantity&availableout=$available");
